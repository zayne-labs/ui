...
export default function DropZoneDemo() {
	return (
		<DropZone.Root
			...
			onUpload={async ({ fileStateArray, onError, onProgress, onSuccess }) => {
				const simulateUpload = (id: string) => {
					...
					const interval = setInterval(() => {
						...
						onProgress({ fileStateOrID: id, progress });
				...
				await Promise.all(
					fileStateArray.map(async ({ id }) => {
						try {
							await simulateUpload(id);
							onSuccess({ fileStateOrID: id });
						} catch (error) {
							onError({ error, fileStateOrID: id });
						}
					})
				);
			}}
