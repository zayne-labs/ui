			...
			>
				<div className="flex flex-col gap-8 py-8">
					<div className="flex items-center gap-3 px-7">
						<div
							className="flex size-9 items-center justify-center rounded-xl bg-indigo-600 font-black
								text-white shadow-lg shadow-indigo-100"
						>
							Z
						</div>
						<span className="text-xl font-black tracking-tight text-slate-900">Zayne UI Dev</span>
					</div>
					...
					<ForWithWrapper
						...
						renderItem={(item) => {
							const isActive =
								currentPath === item.path || (item.path === "/one" && currentPath === "/");
							return (
								<a
									key={item.path}
									href={item.path}
